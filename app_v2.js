...
// Geo Trainer V2 — gameplay (clean rebuild)
(function(){
  ...
  function renderLB(mode, latestWhen){
    ...
    list.forEach((it,idx)=>{
      ...
      const cells=[
        idx+1,
        fmtMS(it.ms),
        new Date(it.when).toISOString().slice(0,10),
        it.stats?.reveals ?? 0,
        it.stats?.bestStreak ?? 0
      ];
  ...
  function startTimer(){
    ...
    tickHandle=setInterval(()=>{ if(timerEl) timerEl.textContent=fmtMS(performance.now()-startTs + penaltyMs); }, 100);
  }
  ...
  function renderQuestion(){
    ...
    if(q.type==='flag-to-country' || q.type==='flag-to-capital'){
      flagWrap.classList.remove('hidden'); flagImg.src=q.flag; // alt handled in HTML; keep decorative here
  ...
  function pushRevealedEntry(q, guessText, displayAnswer){
    ...
    if(q.type==='flag-to-country' || q.type==='flag-to-capital'){
      const img=document.createElement('img'); img.src=q.flag; img.alt=''; img.setAttribute('role','presentation'); img.setAttribute('aria-hidden','true'); tdQ.appendChild(img);
  ...
  function revealCurrent(){
    ...
    const disp=q.label || corr;
    reveals++; penaltyMs += 5000;
    const info=buildUnionValidExcludingUsed(q);
    consumeEntityForCluster(q, info, toKey(q.label));
    pushRevealedEntry(q, input.value, disp);
    if(revealsEl) revealsEl.textContent=reveals;
    feedback.textContent='Revealed (+5s penalty).'; feedback.className='feedback bad';
    updateRibbonPills(); nextQuestion();
  }
  ...
  function nextQuestion(){
    ...
    if(qIndex>=QUEUE.length){
      ...
      const elapsed=performance.now()-startTs + penaltyMs;
      addLB(MODE, elapsed, {reveals, bestStreak});
      saveMistakes(MODE);
      feedback.textContent=`Done — ${fmtMS(elapsed)}.`; feedback.className='feedback ok';
      input.disabled=true; revealBtn.disabled=true; updateRibbonPills(); return;
    }
    renderQuestion();
  }
  ...
  function submitAnswer(val){
    const q=QUEUE[qIndex]; if(!q) return;
    const info = buildUnionValidExcludingUsed(q);
    const set = info.valid; const norm = toKey(val);
    let entity=null;
    if(info && info.rows && info.rows.length){
      for(const r of info.rows){ const ek=entityKeyForRow(r,q.expect); const aset=answerSetForRow(r,q.expect); if(aset.has(norm)){ entity=ek; break; } }
      if(entity && info.used && info.used.has(entity)){
        feedback.textContent='Already used that answer for this set — try its twin.'; feedback.className='feedback bad'; streak=0; updateCounters(); return;
      }
    }
    if(set.has(norm)){
