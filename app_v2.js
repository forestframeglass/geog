...
(function(){
 ...
 // Use your Supabase project's **Anon (legacy) JWT** here (Settings → API → Legacy API keys → anon key)
 ...
 async function getStartToken(mode){
  try{
    if(!SUPABASE_ANON_JWT || String(SUPABASE_ANON_JWT).indexOf('.')<0) return null;
 ...
 function submitGlobalResult(name, mode, elapsedMs, stats){
  try{
    ...
    if(!SUPABASE_ANON_JWT || String(SUPABASE_ANON_JWT).indexOf('.')<0) return;
    fetch(SUPABASE_BASE + '/functions/v1/submit', {
      ...
      body: JSON.stringify({ token: START_TOKEN, name: name, mode: mode, ms: Math.floor(elapsedMs), stats: stats||{} })
    }).catch(function(){});
  }catch(_){ }
 }
 // =======================================
 ...
 function makeQueue(mode, rows){ var q=[]; rows.forEach(function(row){ var countrySet=answerSetForRow(row,'country'); var capitalSet=answerSetForRow(row,'capital');
  if(mode==='flag-to-country'){ if(row.flag) q.push({type:mode, flag:row.flag, flagDupGroup:row.flagDupGroup, capDupGroup:row.capDupGroup, expect:'country', countrySet:countrySet, capitalSet:capitalSet, label:row.country}); }
  else if(mode==='flag-to-capital'){ if(row.flag) q.push({type:mode, flag:row.flag, flagDupGroup:row.flagDupGroup, capDupGroup:row.capDupGroup, expect:'capital', countrySet:countrySet, capitalSet:capitalSet, label:row.capital}); }
  else if(mode==='capital-to-country'){ q.push({type:mode, flagDupGroup:row.flagDupGroup, capDupGroup:row.capDupGroup, prompt:row.capital, expect:'country', countrySet:countrySet, capitalSet:capitalSet, label:row.country}); }
  else if(mode==='country-to-capital'){ q.push({type:mode, flagDupGroup:row.flagDupGroup, capDupGroup:row.capDupGroup, prompt:row.country, expect:'capital', countrySet:countrySet, capitalSet:capitalSet, label:row.capital}); }
 }); return shuffle(q); }

 function buildUnionValidExcludingUsed(q){
  var original = (q.expect==='country'? q.countrySet : q.capitalSet);
  var key = clusterKey(q);
  if(!key) return { valid: original, rows: [], used:new Set(), key:null, clusterLabel:null };
  var rows=[], clusterLabel=null;
  if(key.indexOf('flagdup::')===0){ var g=key.slice('flagdup::'.length); rows = DATA.filter(function(r){ return toKey(r.flagDupGroup||'')===g; }); clusterLabel=q.flagDupGroup; }
  else if(key.indexOf('flag::')===0){ var p=key.slice('flag::'.length); rows = DATA.filter(function(r){ return r.flag===p; }); clusterLabel=''; }
  else if(key.indexOf('capdup::')===0){ var g2=key.slice('capdup::'.length); rows = DATA.filter(function(r){ return toKey(r.capDupGroup||'')===g2; }); clusterLabel=q.capDupGroup; }
  else if(key.indexOf('cap::')===0){ var cap=key.slice('cap::'.length); rows = DATA.filter(function(r){ return toKey(r.capital)===cap; }); clusterLabel=q.prompt; }
  var used = USED_BY_CLUSTER[key] || new Set();
  var union = new Set();
  rows.forEach(function(r){ var ek=entityKeyForRow(r,q.expect); var aset=answerSetForRow(r,q.expect); if(!used.has(ek)){ aset.forEach(function(v){ union.add(v); }); } });
  return { valid: (union.size? union : original), rows:rows, used:used, key:key, clusterLabel:clusterLabel };
 }
 function consumeEntityForCluster(q, info, chosenNorm){
  ...
  for(var i=0;i<info.rows.length;i++){
    var r=info.rows[i];
    var aset=answerSetForRow(r,q.expect);
    if(aset.has(chosenNorm)) { ek=entityKeyForRow(r,q.expect); break; }
  }
  if(!ek) ek = toKey(q.label);
  if(!USED_BY_CLUSTER[info.key]) USED_BY_CLUSTER[info.key]=new Set();
 ...
 function resetCounters(total){
  ...
  for(var k in USED_BY_CLUSTER) delete USED_BY_CLUSTER[k];
  if(qTotalEl) qTotalEl.textContent=total;
  updateCounters();
  if(revealedBody) revealedBody.innerHTML='';
 ...
 function renderQuestion(){
  var q=QUEUE[qIndex]; if(!q) return;
  qShownTs=performance.now();
  var info = buildUnionValidExcludingUsed(q);
  if(dupBadge){
  ...
  if(q.type==='flag-to-country' || q.type==='flag-to-capital'){
    if(flagWrap) flagWrap.classList.remove('hidden');
    if(flagImg) flagImg.src=q.flag;
    if(promptEl) promptEl.textContent = (q.type==='flag-to-country')? 'Which country is this flag?' : "What is the capital of this flag's country?";
  } else {
    if(flagWrap) flagWrap.classList.add('hidden');
    if(promptEl){
 ...
 function pushRevealedEntry(q, guessText, displayAnswer){
  if(!revealedBody) return;
  var row = document.createElement('tr');
  var tdQ=document.createElement('td');
  if(q.type==='flag-to-country' || q.type==='flag-to-capital'){
    var img=document.createElement('img'); img.src=q.flag; img.alt=''; img.setAttribute('role','presentation'); img.setAttribute('aria-hidden','true'); tdQ.appendChild(img);
  } else { tdQ.textContent=q.prompt||''; }
  var tdA=document.createElement('td'); tdA.textContent=displayAnswer||'';
  var tdG=document.createElement('td'); tdG.textContent=guessText||'';
 ...
 function revealCurrent(){
  var q=QUEUE[qIndex]; if(!q) return;
  var corr=q.expect==='country'? Array.from(q.countrySet)[0] : Array.from(q.capitalSet)[0];
  var disp=q.label||corr;
  reveals++; penaltyMs += 5000;
  var info=buildUnionValidExcludingUsed(q);
  consumeEntityForCluster(q, info, toKey(q.label));
  pushRevealedEntry(q, input.value, disp);
  if(revealsEl) revealsEl.textContent=reveals;
  if(penaltyChipEl){ penaltyChipEl.classList.add('show'); setTimeout(function(){ penaltyChipEl.classList.remove('show'); }, 1000); }
  if(feedback){ feedback.textContent='Revealed (+5s penalty).'; feedback.className='feedback bad'; }
  updateRibbonPills();
  nextQuestion();
 }
 ...
 function nextQuestion(){
  ...
  if(qIndex>=QUEUE.length){
    finished=true;
    stopTimer();
    var elapsed=performance.now()-startTs + penaltyMs;
    addLB(MODE, elapsed, {reveals:reveals, bestStreak:bestStreak});
    saveMistakes(MODE);

    // Global submit (best effort)
    try{
      var storedName = localStorage.getItem('gt.v2.playerName') || '';
      var nm = storedName || prompt('Enter your name for the global leaderboard (optional):','');
      nm = (nm||'').trim();
      if(nm) localStorage.setItem('gt.v2.playerName', nm);
      if(nm) submitGlobalResult(nm, MODE, elapsed, {reveals:reveals, bestStreak:bestStreak});
    }catch(_){ }

    if(feedback){ feedback.textContent='Done — '+fmtMS(elapsed)+'.'; feedback.className='feedback ok'; }
 ...
 function submitAnswer(val){
  ...
  if(info && info.rows && info.rows.length){
    for(var i=0;i<info.rows.length;i++){
      var r=info.rows[i];
      var ek=entityKeyForRow(r,q.expect);
      var aset=answerSetForRow(r,q.expect);
      if(aset.has(norm)){ entity=ek; break; }
    }
    if(entity && info.used && info.used.has(entity)){
      if(feedback){ feedback.textContent='Already used that answer for this set — try its twin.'; feedback.className='feedback bad'; }
 ...
 function saveMistakes(mode){
  try{
    var payload = { mode:mode, when: Date.now(), runId: runId, items: wrongLog };
 ...
 // Ctrl/Cmd+R = Reveal (+5s), stop browser refresh
 document.addEventListener('keydown', function(e){
  var k=e.key?e.key.toLowerCase():'';
  if(k==='r' && (e.ctrlKey || e.metaKey)){
    e.preventDefault();
    if(!finished) revealCurrent();
  }
 });
 ...
 function startGame(mode){
  ...
  // mint global start token (best effort)
  START_TOKEN=null;
  getStartToken(mode).then(function(t){ START_TOKEN=t; }).catch(function(){});
 ...
 if(m && MODES.indexOf(m)>=0) MODE=m;
 var btn = document.querySelector('.modes button[data-mode="'+MODE+'"]');
