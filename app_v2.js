...
\
 ...
 // Helpers
 function toKey(s){ return (s\
\
'').toString().trim().toLowerCase().replace(/\\s+/g,' '); }
 function asSet(arr){ const s=new Set(); for(const a of (arr\
\
[])) s.add(toKey(a)); return s; }
...
\
 ...
 function loadLB(mode){ try{ return JSON.parse(localStorage.getItem(lbKey(mode))) \
\
 []; }catch{return []; } }
 ...
 const tds=[idx+1, fmtMS(it.ms), new Date(it.when).toISOString().slice(0,10), it.stats.reveals, it.stats.bestStreak \
\
 0].map(v=>{const td=document.createElement('td'); td.textContent=String(v); return td;});
 tds.forEach(td=>tr.appendChild(td)); leaderboardBody.appendChild(tr); }); }
 function clusterKey(q){
 if(q.type==='flag-to-country' \
\
 q.type==='flag-to-capital'){
 ...
 function answerSetForRow(row, expect){ return expect==='country'? asSet([row.country, ...(row.countryAlt \
\
 [])]) : asSet([row.capital, ...(row.capitalAlt \
\
 [])]); }
 ...
 if(key.startsWith('flagdup::')){ const g=key.slice('flagdup::'.length); rows = DATA.filter(r=> toKey(r.flagDupGroup \
\
 '')===g); clusterLabel=q.flagDupGroup; }
 else if(key.startsWith('flag::')){ const p=key.slice('flag::'.length); rows = DATA.filter(r=> r.flag===p); clusterLabel=''; }
 else if(key.startsWith('capdup::')){ const g=key.slice('capdup::'.length); rows = DATA.filter(r=> toKey(r.capDupGroup \
\
 '')===g); clusterLabel=q.capDupGroup; }
 else if(key.startsWith('cap::')){ const cap=key.slice('cap::'.length); rows = DATA.filter(r=> toKey(r.capital)===cap); clusterLabel=q.prompt; }
 const used = USED_BY_CLUSTER[key] \
\
 new Set();
 const union = new Set();
...
\
 ...
 USED_BY_CLUSTER[info.key] = USED_BY_CLUSTER[info.key] \
\
 new Set();
...
\
 ...
 function revealCurrent(){ const q=QUEUE[qIndex]; if(!q) return; const corr=q.expect==='country'?[...q.countrySet][0]:[...q.capitalSet][0]; const disp=q.label \
\
 corr; reveals++; penaltyMs += 5000; const info=buildUnionValidExcludingUsed(q); consumeEntityForCluster(q, info, toKey(q.label)); pushRevealedEntry(q, input.value, disp); if(revealsEl) revealsEl.textContent=reveals; feedback.textContent='Revealed (+5s penalty).'; feedback.className='feedback bad'; updateRibbonPills(); nextQuestion(); }
 ...
 wrongLog.push({ type:q.type, prompt:q.prompt||null, flag:q.flag||null, expect:q.expect, correct:q.label||'', guess:val||'', when: Date.now() }); saveMistakesDraft(MODE); feedback.textContent='Not quite.'; feedback.className='feedback bad'; streak=0; updateCounters();
 }
 }
 function saveMistakes(mode){
  try{ const payload = { mode, when: Date.now(), runId, items: wrongLog }; localStorage.setItem('gt.v2.mistakes.latest', JSON.stringify(payload)); localStorage.setItem('gt.v2.mistakes.'+mode+'.latest', JSON.stringify(payload)); }catch(_){ }
 }
 function saveMistakesDraft(mode){
  try{ const payload = { mode, when: Date.now(), runId, items: wrongLog }; localStorage.setItem('gt.v2.mistakes.draft', JSON.stringify(payload)); localStorage.setItem('gt.v2.mistakes.'+mode+'.draft', JSON.stringify(payload)); }catch(_){ }
 }
 // Events
 ...
 document.addEventListener('keydown', e=>{ const k=e.key?e.key.toLowerCase():''; if(k==='r' && (e.ctrlKey \
\
 e.metaKey)){ e.preventDefault(); if(!finished) revealCurrent(); }});
 function startGame(mode){ QUEUE=makeQueue(mode, DATA); penaltyMs=0; wrongLog=[]; runId=Date.now(); try{localStorage.removeItem('gt.v2.mistakes.draft');}catch(_){} resetCounters(QUEUE.length); renderLB(mode); renderQuestion(); startTimer(); }
