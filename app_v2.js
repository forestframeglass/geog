...
(function(){
 ...
 function nextQuestion(){ if(finished) return; qIndex++; if(qIndex>=QUEUE.length){ finished=true; stopTimer(); var elapsed=performance.now()-startTs + penaltyMs; addLB(MODE, elapsed, {reveals:reveals, bestStreak:bestStreak}); saveMistakes(MODE);
   ...
   if(input) input.disabled=true; if(revealBtn) revealBtn.disabled=true; updateRibbonPills(); return; }
 renderQuestion(); }
 ...
 // ===== NEW: mint start token asynchronously (best effort) =====
 START_TOKEN = null; getStartToken(mode).then(t=>{ START_TOKEN=t; }).catch(()=>{});
 }
