...
// Geo Trainer V2 — gameplay (stable build)
(function(){
  ...
  function renderLB(mode, latestWhen){ if(!leaderboardBody) return; const list=loadLB(mode); leaderboardBody.innerHTML=''; list.forEach((it,idx)=>{ const tr=document.createElement('tr'); if(latestWhen && it.when===latestWhen) tr.classList.add('latest'); const timeStr = fmtMS(it.ms) + ((it.stats && it.stats.reveals>0)? '*':'' ); const cells=[ idx+1, timeStr, new Date(it.when).toISOString().slice(0,10), it.stats?.reveals??0, it.stats?.bestStreak??0 ]; cells.forEach(v=>{ const td=document.createElement('td'); td.textContent=String(v); tr.appendChild(td); }); leaderboardBody.appendChild(tr); }); }

  function clusterKey(q){ if(q.type==='flag-to-country' || q.type==='flag-to-capital'){ if(q.flagDupGroup) return 'flagdup::'+toKey(q.flagDupGroup); if(q.flag) return 'flag::'+q.flag; } if(q.type==='capital-to-country'){ if(q.capDupGroup) return 'capdup::'+toKey(q.capDupGroup); if(q.prompt) return 'cap::'+toKey(q.prompt); } return null; }
  ...
  function makeQueue(mode, rows){ const q=[]; for(const row of rows){ const countrySet=answerSetForRow(row,'country'); const capitalSet=answerSetForRow(row,'capital'); if(mode==='flag-to-country'){ if(row.flag) q.push({type:mode, flag:row.flag, flagDupGroup:row.flagDupGroup, capDupGroup:row.capDupGroup, expect:'country', countrySet, capitalSet, label:row.country}); } else if(mode==='flag-to-capital'){ if(row.flag) q.push({type:mode, flag:row.flag, flagDupGroup:row.flagDupGroup, capDupGroup:row.capDupGroup, expect:'capital', countrySet, capitalSet, label:row.capital}); } else if(mode==='capital-to-country'){ q.push({type:mode, flagDupGroup:row.flagDupGroup, capDupGroup:row.capDupGroup, prompt:row.capital, expect:'country', countrySet, capitalSet, label:row.country}); } else if(mode==='country-to-capital'){ q.push({type:mode, flagDupGroup:row.flagDupGroup, capDupGroup:row.capDupGroup, prompt:row.country, expect:'capital', countrySet, capitalSet, label:row.capital}); } } return shuffle(q); }
  function buildUnionValidExcludingUsed(q){ const original = (q.expect==='country'? q.countrySet : q.capitalSet); const key = clusterKey(q); if(!key) return { valid: original, rows: [], used:new Set(), key:null, clusterLabel:null }; let rows=[]; let clusterLabel=null; if(key.startsWith('flagdup::')){ const g=key.slice('flagdup::'.length); rows = DATA.filter(r=> toKey(r.flagDupGroup??'')===g); clusterLabel=q.flagDupGroup; } else if(key.startsWith('flag::')){ const p=key.slice('flag::'.length); rows = DATA.filter(r=> r.flag===p); clusterLabel=''; } else if(key.startsWith('capdup::')){ const g=key.slice('capdup::'.length); rows = DATA.filter(r=> toKey(r.capDupGroup??'')===g); clusterLabel=q.capDupGroup; } else if(key.startsWith('cap::')){ const cap=key.slice('cap::'.length); rows = DATA.filter(r=> toKey(r.capital)===cap); clusterLabel=q.prompt; } const used = USED_BY_CLUSTER[key] ?? new Set(); const union = new Set(); for(const r of rows){ const ek=entityKeyForRow(r,q.expect); const aset=answerSetForRow(r,q.expect); if(!used.has(ek)){ for(const v of aset) union.add(v); } } return { valid: (union.size? union : original), rows, used, key, clusterLabel }; }
  function consumeEntityForCluster(q, info, chosenNorm){ if(!info || !info.key) return; let ek=null; for(const r of info.rows){ const aset=answerSetForRow(r,q.expect); if(aset.has(chosenNorm)){ ek=entityKeyForRow(r,q.expect); break; } } if(!ek) ek = toKey(q.label); USED_BY_CLUSTER[info.key] = USED_BY_CLUSTER[info.key] ?? new Set(); USED_BY_CLUSTER[info.key].add(ek); }
  ...
  function renderQuestion(){ const q=QUEUE[qIndex]; if(!q) return; qShownTs=performance.now(); const info = buildUnionValidExcludingUsed(q); if(info && info.rows && info.rows.length>1){ dupBadge?.classList.remove('hidden'); dupBadge.textContent = "multiple answers available, don't reuse the same one"; } else dupBadge?.classList.add('hidden'); if(q.type==='flag-to-country' || q.type==='flag-to-capital'){ flagWrap?.classList.remove('hidden'); if(flagImg) { flagImg.src=q.flag; } promptEl.textContent = (q.type==='flag-to-country')? 'Which country is this flag?' : "What is the capital of this flag's country?"; } else { flagWrap?.classList.add('hidden'); if(q.type==='capital-to-country') promptEl.innerHTML = `Capital: <strong>${q.prompt}</strong> — which country?`; else promptEl.innerHTML = `Country: <strong>${q.prompt}</strong> — what is the capital?`; } input.value=''; input.disabled=false; revealBtn.disabled=false; input.focus(); updateCounters(); }

  function pushRevealedEntry(q, guessText, displayAnswer){ if(!revealedBody) return; const row = document.createElement('tr'); const tdQ=document.createElement('td'); if(q.type==='flag-to-country' || q.type==='flag-to-capital'){ const img=document.createElement('img'); img.src=q.flag; img.alt=''; img.setAttribute('role','presentation'); img.setAttribute('aria-hidden','true'); tdQ.appendChild(img); } else tdQ.textContent=q.prompt??''; const tdA=document.createElement('td'); tdA.textContent=displayAnswer??''; const tdG=document.createElement('td'); tdG.textContent=guessText??''; row.appendChild(tdQ); row.appendChild(tdA); row.appendChild(tdG); revealedBody.prepend(row); }

  function revealCurrent(){ const q=QUEUE[qIndex]; if(!q) return; const corr=q.expect==='country'?[...q.countrySet][0]:[...q.capitalSet][0]; const disp=q.label||corr; reveals++; penaltyMs += 5000; const info=buildUnionValidExcludingUsed(q); consumeEntityForCluster(q, info, toKey(q.label)); pushRevealedEntry(q, input.value, disp); if(revealsEl) revealsEl.textContent=reveals; if(penaltyChipEl){ penaltyChipEl.classList.add('show'); setTimeout(()=> penaltyChipEl.classList.remove('show'), 1000); } feedback.textContent='Revealed (+5s penalty).'; feedback.className='feedback bad'; updateRibbonPills(); nextQuestion(); }

  function nextQuestion(){ if(finished) return; qIndex++; if(qIndex>=QUEUE.length){ finished=true; stopTimer(); const elapsed=performance.now()-startTs + penaltyMs; addLB(MODE, elapsed, {reveals, bestStreak}); saveMistakes(MODE); feedback.textContent=`Done — ${fmtMS(elapsed)}.`; feedback.className='feedback ok'; input.disabled=true; revealBtn.disabled=true; updateRibbonPills(); return; } renderQuestion(); }

  function submitAnswer(val){ const q=QUEUE[qIndex]; if(!q) return; const info = buildUnionValidExcludingUsed(q); const set = info.valid; const norm = toKey(val); let entity=null; if(info && info.rows && info.rows.length){ for(const r of info.rows){ const ek=entityKeyForRow(r,q.expect); const aset=answerSetForRow(r,q.expect); if(aset.has(norm)){ entity=ek; break; } } if(entity && info.used && info.used.has(entity)){ feedback.textContent='Already used that answer for this set — try its twin.'; feedback.className='feedback bad'; streak=0; updateCounters(); return; } } if(set.has(norm)){ correct++; streak++; bestStreak=Math.max(bestStreak, streak); feedback.textContent='Correct.'; feedback.className='feedback ok'; consumeEntityForCluster(q, info, norm); updateRibbonPills(); nextQuestion(); } else { wrongLog.push({ type:q.type, prompt:q.prompt||null, flag:q.flag||null, expect:q.expect, correct:q.label||'', guess:val||'', when: Date.now() }); saveMistakesDraft(MODE); if(hudMistakes) hudMistakes.textContent = `Mistakes (${wrongLog.length})`; feedback.textContent='Not quite.'; feedback.className='feedback bad'; streak=0; updateCounters(); } }

  function saveMistakes(mode){ try{ const payload = { mode, when: Date.now(), runId, items: wrongLog }; localStorage.setItem('gt.v2.mistakes.latest', JSON.stringify(payload)); localStorage.setItem('gt.v2.mistakes.'+mode+'.latest', JSON.stringify(payload)); }catch(_){ } }
  function saveMistakesDraft(mode){ try{ const payload = { mode, when: Date.now(), runId, items: wrongLog }; localStorage.setItem('gt.v2.mistakes.draft', JSON.stringify(payload)); localStorage.setItem('gt.v2.mistakes.'+mode+'.draft', JSON.stringify(payload)); }catch(_){ } }
  ...
  const m = new URLSearchParams(location.search).get('mode'); if(m && MODES.includes(m)) MODE=m; const btn = document.querySelector(`.modes button[data-mode="${MODE}"]`); if(btn){ btn.classList.add('active'); }
  startGame(MODE);
})();
